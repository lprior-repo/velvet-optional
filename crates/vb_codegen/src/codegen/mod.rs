...
pub use types::{CodegenError, CodegenResult};
pub use structs::{ExpressionStoreMetrics, ForEachStartEmit, ListStoreMetrics, ValueStoreMetrics};
...
fn emit_wait_event_suspend(
    ...
    match next {
        Some(resume_pc) => writeln!(out, "    Err(SuspensionOutcome::WaitEvent {{ step: {}, event_slot: {}, timeout_slot: {}, resume_pc: {} }}.into_drive_error())", step.get(), event.get(), timeout_slot.map_or("None".to_string(), |s| format!("Some({})", s.get())), resume_pc.get()).map_err(fmt_err),
        None => writeln!(out, "    Err(DriveError::MissingNextStep)").map_err(fmt_err),
    }
}
...
fn emit_ask_suspend(
    ...
    match next {
        Some(resume_pc) => writeln!(out, "    Err(SuspensionOutcome::AskPending {{ step: {}, prompt_slot: {}, timeout_slot: {}, resume_pc: {} }}.into_drive_error())", step.get(), prompt.get(), timeout_slot.map_or("None".to_string(), |s| format!("Some({})", s.get())), resume_pc.get()).map_err(fmt_err),
        None => writeln!(out, "    Err(DriveError::MissingNextStep)").map_err(fmt_err),
    }
}

fn emit_optional_timeout_read(out: &mut String, timeout_slot: Option<SlotIdx>) -> CodegenResult<()> {
    if let Some(slot) = timeout_slot {
        writeln!(out, "    let _timeout = read_slot(slots, {})?;", slot.get()).map_err(fmt_err)?;
    }
    Ok(())
}
...
fn emit_ask_resume_step(
    ...
    match next {
        Some(resume_pc) => writeln!(out, "    Ok(StepOutcome::Continue({}))", resume_pc.get()).map_err(fmt_err),
        None => writeln!(out, "    Err(DriveError::MissingNextStep)").map_err(fmt_err),
    }
}
...
pub fn emit_expr_function(
    ...
    writeln!(
        out,
        "    let mut stack = ExprStack::new(CONTRACT_MAX_EXPR_STACK)?;"
    )
    .map_err(fmt_err)?;
    ...
    for op in program.ops.as_ref() {
        match op {
            ExprOp::LoadSlot(slot) => {
                writeln!(out, "    stack.push_tainted(read_slot(slots, {})?, read_taint(slot_taints, {})?)?;", slot.get(), slot.get()).map_err(fmt_err)?;
            }
            ExprOp::LoadConst(idx) => {
                writeln!(out, "    let _ = stack.push(read_const({})?);", idx.get()).map_err(fmt_err)?;
            }
            ExprOp::LoadAccessor(accessor) => {
                emit_accessor_eval(out, *accessor, workflow)?;
            }
            ExprOp::Eq => {
                writeln!(out, "    {{ let (_rhs, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_lhs, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _v = match (_lhs, _rhs) {{ (SlotValue::Null, SlotValue::Null) => SlotValue::Bool(true), (SlotValue::Bool(a), SlotValue::Bool(b)) => SlotValue::Bool(a == b), (SlotValue::I64(a), SlotValue::I64(b)) => SlotValue::Bool(a == b), (SlotValue::F64(a), SlotValue::F64(b)) => SlotValue::Bool(a == b), (SlotValue::Symbol(a), SlotValue::Symbol(b)) => SlotValue::Bool(a == b), _ => SlotValue::Bool(false) }}; stack.push_tainted(_v, join_taint(_lt, _rt))?; }}").map_err(fmt_err)?;
            }
            ExprOp::NotEq => {
                writeln!(out, "    {{ let (_rhs, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_lhs, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _v = match (_lhs, _rhs) {{ (SlotValue::Null, SlotValue::Null) => SlotValue::Bool(false), (SlotValue::Bool(a), SlotValue::Bool(b)) => SlotValue::Bool(a != b), (SlotValue::I64(a), SlotValue::I64(b)) => SlotValue::Bool(a != b), (SlotValue::F64(a), SlotValue::F64(b)) => SlotValue::Bool(a != b), (SlotValue::Symbol(a), SlotValue::Symbol(b)) => SlotValue::Bool(a != b), _ => SlotValue::Bool(true) }}; stack.push_tainted(_v, join_taint(_lt, _rt))?; }}").map_err(fmt_err)?;
            }
            ExprOp::Gt => {
                writeln!(out, "    {{ let (_rhs, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_lhs, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _v = match (_lhs, _rhs) {{ (SlotValue::I64(a), SlotValue::I64(b)) => SlotValue::Bool(a > b), (SlotValue::F64(a), SlotValue::F64(b)) => SlotValue::Bool(a > b), _ => return Err(DriveError::TypeMismatch {{ expected: \"number\", found: \"neither i64 nor f64\" }}) }}; stack.push_tainted(_v, join_taint(_lt, _rt))?; }}").map_err(fmt_err)?;
            }
            ExprOp::Gte => {
                writeln!(out, "    {{ let (_rhs, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_lhs, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _v = match (_lhs, _rhs) {{ (SlotValue::I64(a), SlotValue::I64(b)) => SlotValue::Bool(a >= b), (SlotValue::F64(a), SlotValue::F64(b)) => SlotValue::Bool(a >= b), _ => return Err(DriveError::TypeMismatch {{ expected: \"number\", found: \"neither i64 nor f64\" }}) }}; stack.push_tainted(_v, join_taint(_lt, _rt))?; }}").map_err(fmt_err)?;
            }
            ...
            ExprOp::And => {
                writeln!(out, "    {{ let (_rhs, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_lhs, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _cond = match (_lhs, _rhs) {{ (SlotValue::Bool(a), SlotValue::Bool(b)) => a && b, _ => return Err(DriveError::TypeMismatch {{ expected: \"boolean\", found: \"non-boolean\" }}) }}; stack.push_tainted(SlotValue::Bool(_cond), join_taint(_lt, _rt))?; }}").map_err(fmt_err)?;
            }
            ExprOp::Or => {
                writeln!(out, "    {{ let (_rhs, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_lhs, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _cond = match (_lhs, _rhs) {{ (SlotValue::Bool(a), SlotValue::Bool(b)) => a || b, _ => return Err(DriveError::TypeMismatch {{ expected: \"boolean\", found: \"non-boolean\" }}) }}; stack.push_tainted(SlotValue::Bool(_cond), join_taint(_lt, _rt))?; }}").map_err(fmt_err)?;
            }
            ExprOp::Not => {
                writeln!(out, "    {{ let (_v, _taint) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _result = match _v {{ SlotValue::Bool(b) => !b, other => return Err(DriveError::TypeMismatch {{ expected: \"boolean\", found: other.type_name() }}), }}; stack.push_tainted(SlotValue::Bool(_result), _taint)?; }}").map_err(fmt_err)?;
            }
            ExprOp::Add => {
                writeln!(out, "    {{ let (_rhs, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_lhs, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _v = match (_lhs, _rhs) {{ (SlotValue::I64(a), SlotValue::I64(b)) => {{ let result = a.checked_add(b).ok_or(DriveError::IntegerOverflow)?; SlotValue::I64(result) }}, (SlotValue::F64(a), SlotValue::F64(b)) => SlotValue::I64(a as i64 + b as i64), _ => return Err(DriveError::TypeMismatch {{ expected: \"number\", found: \"neither i64 nor f64\" }}) }}; stack.push_tainted(_v, join_taint(_lt, _rt))?; }}").map_err(fmt_err)?;
            }
            ExprOp::Sub => {
                writeln!(out, "    {{ let (_rhs, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_lhs, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _v = match (_lhs, _rhs) {{ (SlotValue::I64(a), SlotValue::I64(b)) => {{ let result = a.checked_sub(b).ok_or(DriveError::IntegerOverflow)?; SlotValue::I64(result) }}, (SlotValue::F64(a), SlotValue::F64(b)) => SlotValue::I64(a as i64 - b as i64), _ => return Err(DriveError::TypeMismatch {{ expected: \"number\", found: \"neither i64 nor f64\" }}) }}; stack.push_tainted(_v, join_taint(_lt, _rt))?; }}").map_err(fmt_err)?;
            }
            ExprOp::Mul => {
                writeln!(out, "    {{ let (_rhs, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_lhs, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _v = match (_lhs, _rhs) {{ (SlotValue::I64(a), SlotValue::I64(b)) => {{ let result = a.checked_mul(b).ok_or(DriveError::IntegerOverflow)?; SlotValue::I64(result) }}, (SlotValue::F64(a), SlotValue::F64(b)) => SlotValue::I64((a * b) as i64), _ => return Err(DriveError::TypeMismatch {{ expected: \"number\", found: \"neither i64 nor f64\" }}) }}; stack.push_tainted(_v, join_taint(_lt, _rt))?; }}").map_err(fmt_err)?;
            }
            ExprOp::Div => {
                writeln!(out, "    {{ let (_rhs, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_lhs, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_v, _t) = match (_lhs, _rhs) {{ (SlotValue::I64(a), SlotValue::I64(b)) => {{ let result = a.checked_div(b).ok_or(DriveError::DivisionByZero)?; (SlotValue::I64(result), join_taint(_lt, _rt)) }}, (SlotValue::F64(a), SlotValue::F64(b)) => (SlotValue::F64(a / b), join_taint(_lt, _rt)), _ => return Err(DriveError::TypeMismatch {{ expected: \"number\", found: \"neither i64 nor f64\" }}) }}; stack.push_tainted(_v, _t)?; }}").map_err(fmt_err)?;
            }
            ExprOp::Has => {
                writeln!(out, "    {{ let (_field, _ft) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_obj, _ot) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _handle = expect_object_value(_obj)?; let _field_u32 = match _field {{ SlotValue::Symbol(s) => s, other => return Err(DriveError::TypeMismatch {{ expected: \"symbol\", found: other.type_name() }}), }}; let _exists = object_store.has_field(_handle, _field_u32)?; stack.push_tainted(SlotValue::Bool(_exists), join_taint(_ot, _ft))?; }}").map_err(fmt_err)?;
            }
            ExprOp::Exists => {
                writeln!(out, "    {{ let (_v, _taint) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; stack.push_tainted(SlotValue::Bool(!matches!(_v, SlotValue::Null)), _taint)?; }}").map_err(fmt_err)?;
            }
            ExprOp::Length => {
                writeln!(out, "    {{ let (_v, _taint) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _len = match _v {{ SlotValue::List(handle) => i64::from(list_item_count(list_store, handle)?), SlotValue::Object(handle) => i64::from(object_field_count(object_store, handle)?), other => return Err(DriveError::TypeMismatch {{ expected: \"list or object\", found: other.type_name() }}), }}; stack.push_tainted(SlotValue::I64(_len), _taint)?; }}").map_err(fmt_err)?;
            }
            ExprOp::Empty => {
                writeln!(out, "    {{ let (_v, _taint) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _is_empty = match _v {{ SlotValue::List(handle) => list_item_count(list_store, handle)? == 0, SlotValue::Object(handle) => object_field_count(object_store, handle)? == 0, SlotValue::Null => true, other => return Err(DriveError::TypeMismatch {{ expected: \"list, object, or null\", found: other.type_name() }}), }}; stack.push_tainted(SlotValue::Bool(_is_empty), _taint)?; }}").map_err(fmt_err)?;
            }
            ExprOp::Append => {
            ...
            ExprOp::Merge => {
                writeln!(out, "    {{ let (_right, _rt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let (_left, _lt) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _left_handle = expect_object_value(_left)?; let _right_handle = expect_object_value(_right)?; let _merged = merge_object_records(object_store, _left_handle, _right_handle)?; stack.push_tainted(SlotValue::Object(_merged), join_taint(_lt, _rt))?; }}").map_err(fmt_err)?;
            }
            ExprOp::Sum => {
                writeln!(out, "    {{ let (_v, _taint) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _handle = expect_list_value(_v)?; let _sum = sum_list_items(list_store, _handle)?; stack.push_tainted(SlotValue::I64(_sum), _taint)?; }}").map_err(fmt_err)?;
            }
            ExprOp::Count => {
                writeln!(out, "    {{ let (_v, _taint) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _result = match _v {{ SlotValue::List(handle) => i64::from(list_item_count(list_store, handle)?), other => return Err(DriveError::TypeMismatch {{ expected: \"list\", found: other.type_name() }}), }}; stack.push_tainted(SlotValue::I64(_result), _taint)?; }}").map_err(fmt_err)?;
            }
            ExprOp::Unique => {
                writeln!(out, "    {{ let (_v, _taint) = stack.pop_tainted().ok_or(DriveError::ExpressionStackUnderflow)?; let _handle = expect_list_value(_v)?; let _unique = unique_list_items(list_store, _handle)?; stack.push_tainted(SlotValue::List(_unique), _taint)?; }}").map_err(fmt_err)?;
            }
...
fn checked_metric_add(a: usize, b: usize, msg: &'static str) -> CodegenResult<usize> {
    a.checked_add(b).ok_or(CodegenError::SemanticMismatch { detail: msg.into() })
}

fn checked_metric_mul(a: usize, b: usize, msg: &'static str) -> CodegenResult<usize> {
    a.checked_mul(b).ok_or(CodegenError::SemanticMismatch { detail: msg.into() })
}
...
fn expression_store_metrics(workflow: &CompiledWorkflow) -> CodegenResult<ExpressionStoreMetrics> {
    ...
    for idx in 0..u16::MAX {
        let expr_idx = vb_core::ExprIdx::new(idx);
        let Some(program) = workflow.expression(expr_idx) else {
            break;
        };
        for op in program.ops.as_ref() {
            match op {
                ExprOp::Append | ExprOp::AppendIf | ExprOp::Sum | ExprOp::Unique => {
                    metrics.list_allocating_ops = checked_metric_add(
                        metrics.list_allocating_ops,
                        1,
                        "expression store metrics overflow",
                    )?;
                }
                ExprOp::Merge => {
